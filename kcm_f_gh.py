...
def gaussian_kernel(xl, xk, hyper_parameters):
    ...
    acc = numpy.sum(numpy.multiply(hyper_parameters, numpy.power(xl - xk, 2)))
    return exp((-0.5) * acc)
    ...
    for i in range(0, number_rows):
        ...
        for j in range(i + 1, number_rows):
            ...
            gaussian_kernel_matrix[i][j] = gaussian_kernel_matrix[j][i] = \
                gaussian_kernel(xi, xj, hyper_parameters)
...
def update_hyper_parameters(data_set, clusters, num_clusters, cluster_sizes, hyper_parameters, gamma):
    ...
    for i in range(0, number_rows):
        ...
        for j in range(i + 1, number_rows):
            xj = data_set.iloc[j]
            g = gaussian_kernel(xi, xj, hyper_parameters)
...
def kcm_f_gh(data_set, num_clusters):
    ...
    clusters, hyper_parameters = update_clusters(
        data_set, clusters, num_clusters, cluster_sizes, hyper_parameters,
        gamma)
    print('clusters')
    print(clusters)
